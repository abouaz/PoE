...
# ---- path setup so we can import model.py from sd-moe ----
SD_MOE_DIR = "$HOME/sd-moe"
sys.path.insert(0, SD_MOE_DIR)
...
def discover_checkpoints(ckpt_dir):
    """Find all step_*/checkpoint.pt, return sorted list of (step, path)."""
    entries = []
    for name in os.listdir(ckpt_dir):
        m = re.match(r"step_(\d+)", name)
        if not m:
            continue
        path = os.path.join(ckpt_dir, name, "checkpoint.pt")
